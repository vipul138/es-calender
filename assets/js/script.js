...
let deleteAllUsers = document.querySelector("#delete-users");
...
function displayUsersInTable() {
  ...
  users.forEach((user, index) => {
    ...
    birthdateCell.innerText = user.birthdate;
...
submit.addEventListener("click", (event) => {
  ...
  if (getName.value === "" || birthdate.value === "") {
...
deleteAllUsers.addEventListener("click", (event) => {
  event.preventDefault();
  users = [];
  displayUsersInTable();
  getName.value = "";
      ...
      if (element.className === days[day]) {
        ...
        if (element.childElementCount > 0) {
          ...
          for (let i = 0; i < child.length; i++) {
            if (
              child[i].getAttribute("data-age") >=
              newli.getAttribute("data-age")
            ) {
    ...
    dayElements.forEach((element) => {
      if (element.childElementCount === 0) {
        ...
        element.nextElementSibling.innerText =
          element.childElementCount + " Birthdays";
